...
class TestTemplateConfigPruning(TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.gemm_configs = [
            GemmConfig(BLOCK_M, BLOCK_N, BLOCK_K, stage, warps)
            for BLOCK_M, BLOCK_N, BLOCK_K, warps, stage in product(
                block_sizes, block_sizes, block_sizes, num_warps, num_stages
            )
        ]
    ...
    def run_op_shared_mem_pruning_check(
        ...
        for c in self.gemm_configs:
            ...
            if triton_compilation_fails:
                ...
                self.assertFalse(
                    exceeds,
                    f"Config {c} compiled successfully, "
                    "but the checker predicted it would exceed shared memory limits.",
                )
                self.assertTrue(
                    abs(captured_smem - smem_estimation) < 25_000,
                    "Estimated smem deviates from actual smem by more than 25KB"
                    f"for config {c}, Actual: {captured_smem} Estimated: {smem_estimation}",
                )
